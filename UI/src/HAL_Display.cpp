...
void initStars() {
    for(int i = 0; i < MAX_STARS; i++) {
        stars[i].x = random(0, u8g2.getWidth());
        stars[i].y = random(0, u8g2.getHeight());
    }
}
...
void showStarfield() {
    ...
    for(int i = 0; i < MAX_STARS; i++) {
        // 随机决定是否显示这颗星星（闪烁效果）
        if(random(100) > 20) {  // 80%的概率显示
            u8g2.drawPixel(stars[i].x, stars[i].y);
        }
        
        // 随机移动星星位置
        if(random(100) < 10) {  // 10%的概率移动
            stars[i].x = random(0, u8g2.getWidth());
            stars[i].y = random(0, u8g2.getHeight());
        }
    }
    u8g2.drawUTF8(23, 16, "APPLE JUICE");
    u8g2.drawUTF8(25, 48, "version 1.1.2");
    u8g2.sendBuffer();
    delay(100);  // 控制刷新速率
}
void showAppleJuice()
